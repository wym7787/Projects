		...
		for(int i=0 ; i<arr.length ; i++)
		{
			...
			if(rs1.next())
			{
				if(rs1.getString("id").equals(arr[i]))
				{
					...
					sql = "insert into user values('"+ id +"','" + name+"','" +nicname+ "','" +password +"','" + email+ "','"+receive +"','" +date+"')";
					st2.executeUpdate(sql);
					sql = "delete from disuser where id='"+id+"'";
					st1.executeUpdate(sql);
					
				}
			}
		}
		out.println("<script>alert('복구 완료 되었습니다');window.location='userInfo.jsp';</script>");
	}
%>
</body>
</html>