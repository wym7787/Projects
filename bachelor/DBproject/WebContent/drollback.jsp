	...
	<div class ="top-content">
		<div class ="toprap">
			<div class ="topsort">
				...
				<div class ="toptext"><h4>의사/간호사 삭제 목록</h4></div>
			</div>	
		</div>
	</div>

<form method = "post" action = "drollback.jsp">
<div id="sign-container">
			<div class ="sign-nav">
				...
				<a class ="button" href="drollback.jsp">전체조회</a>
				</div>
			</div>
...
<div id ="button-sort">
	<a class = "button" href="dsearch.jsp">의사/간호사 목록</a>
	<a class = "button" onclick="ddelete2();">영구삭제</a>
	<a class = "button" onclick="drollback2();">복구</a>
</div>
...
<table id = "container-table">
			...
			<%
				...
				String sql = "select * from disdoctor order by certinum"; // default
				...
				if(!certinum.equals("") && name.equals("") && job.equals(""))
				{
					...
					sql = "select * from disdoctor where certinum = '" + certinum + "'";
				...
				if(!certinum.equals("") && !name.equals("") && job.equals(""))
				{
					...
					sql = "select * from disdoctor where certinum = '" + certinum + "' and name = '" + name + "'";
					rs = st.executeQuery(sql);
					while(rs.next())
					{
				...
				if(!certinum.equals("") && name.equals("") && !job.equals(""))
				{
					...
					sql = "select * from disdoctor where certinum = '" + certinum + "' and job = '" + job +"'";
					rs = st.executeQuery(sql);
					while(rs.next())
					{
				...
				if(certinum.equals("") && !name.equals("") && job.equals(""))
				{
					...
					sql = "select * from disdoctor where name = '" + name + "'";
					rs = st.executeQuery(sql);
					while(rs.next())
					{
				...
				if(!certinum.equals("") && !name.equals("") && !job.equals(""))
				{
					...
					sql = "select * from disdoctor where name = '" + certinum + "' and job = '" + job +"'";
					rs = st.executeQuery(sql);
					while(rs.next())
					{
				...
				if(certinum.equals("") && name.equals("") && !job.equals(""))
				{
					...
					sql = "select * from disdoctor where job = '" + job + "'";
					rs = st.executeQuery(sql);
					while(rs.next())
					{
